...
<body>
  ...
  <a class="navbar-brand" href="index.php"><div id="datetime" style=""></b></h2><script>setInterval("document.getElementById('datetime').innerHTML=new Date().toLocaleString();", 1000);</script></div></a>
...
<div class="card mb-3" id="card-<?php echo $v['id'];?>" data-id="<?php echo $v['id']?>" style="font-size:14px;">
     <div class="card-header<?php echo $v['aid']==1?' text-danger':''?>"><?php echo view_ip($v['author'],$v['mail']);?>&nbsp&nbsp<?php echo timeago($v['addtime']);?><br/>IP:<?php echo view_ip($v['ip'],$v['mail']);?><?php if($v['top']==1) echo '<span style="color:#F00;">[置顶]</span>';?><?php if($v['lock']==1) echo '<span style="color:#F00;">[未审核]</span>';?><span class="p-1"><a data-act="reply" class="click float-right" href="javascript:;">回复</a></span></div>
...
?>      
  ...
  ?>
   ...
   <!-- </div>-->  
<footer class="p-5 text-center">Powered by <a href="https://bonehunter.cn/" target="_blank">小骨猎人</a> Talk  首开五壮士 Processed <?php $t2 = microtime(true); echo round($t2-$t1,3); ?>s</footer>   
<!--对话框-->
<!-- 模态框 -->
